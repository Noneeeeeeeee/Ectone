...
local Ectone = game:GetObjects("rbxassetid://10804731440")[1]

Ectone.Enabled = false


if gethui then
	Ectone.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Ectone)
	Ectone.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Ectone.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Ectone.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Ectone.Name and Interface ~= Ectone then
			Interface.Enabled = false
			Interface.Name = "Ectone-Old"
		end
	end
else
...
local Main = Ectone.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Ectone.DisplayOrder = 100
...
function ChangeTheme(ThemeName)
	...
	Ectone.Main.BackgroundColor3 = SelectedTheme.Background
	Ectone.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Ectone.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Ectone.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Ectone.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Ectone.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Ectone.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if EctoneLibrary.Flags[FlagName] then
			...
			EctoneLibrary:Notify({Title = "Flag Error", Content = "Ectone was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Ectone | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function EctoneLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Ectone | "
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Ectone" then
		LoadingFrame.Version.Text = "Ectone UI"
	end
	...
	if Settings.Discord then
		...
		if not isfile(EctoneFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then 
				writefile(EctoneFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Ectone RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Ectone | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Ectone | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Ectone | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Ectone | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then 
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Ectone | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Ectone | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Ectone | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
